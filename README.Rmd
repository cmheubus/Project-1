...
In a [shutout game](https://en.wikipedia.org/wiki/Shutout), a hockey team's defense manages to prevent the other team from scoring any points throughout the entirety of the game - surely, a sign of strong performance! To begin, I decided to created a categorical variable called **shutoutRange**, which defined each 
...
```{r Creating a Numeric Summary of the Shutouts}
#shutoutsSummary<- team_totals_data$shutouts %>%
...
I created a ratio 
Ratio of RoadWin:RoadLoss and HomeWin:HomeLoss, then do 
Did not count ties
...
team_totals_data <- mutate(team_totals_data, percentLossAway = roadWins / wins)
team_totals_data <- mutate(team_totals_data, percentWinAway = homeWins / wins)
team_totals_data %>%
  select(teamName, roadWinLossRatio, homeWinLossRatio) %>%
  arrange(desc(roadWinLossRatio)) %>%
  head()
```
...
Looking at the mean and median win-to-loss ratios for on the road (or "away") and home, we see that both the home mean and median ratios are nearly double for home games - perhaps indicating that a time is more likely to win when it's playing on its home ice. We can then look at boxplots of these ratios, as a visual representation of these summary statistics.  
...
```{r Creating Boxplot of home WinLossRatio}
...
```{r Creating Histogram of Home Wins to Losses Ratio}
...
## How are Weak Goalies Affecting a Team's Performance? 
