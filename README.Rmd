...
goalieCanes<- api_function_goalie(26) #Testing the function with Carolina Hurricanes data, to honor my "new home"! Maybe I'll even use their colors later on...
```
...
```{r}
#newTotals <- team_totals_table %>%
 #select(teamName, gamesPlayed, wins, losses)
```

## Is an Team With More Recorded Shutouts More Likely to Still be an Active Franchise? 

```{r}
team_totals_table <- table(team_totals_data$activeFranchise, team_totals_data$firstSeasonId)
kable(team_totals_table)
```
...
shutoutsSummary<- summary(team_totals_data$shutouts)
shutoutsSummary
```
...
Did not count ties
```{r Create roadWinToLossRatio variable}
team_totals_data <- mutate(team_totals_data, roadWinToLossRatio = roadWins / roadLosses)
team_totals_data <- mutate(team_totals_data, homeWinToLossRatio = homeWins / homeLosses)
team_totals_data
```

```{r}
penalty_plot <- ggplot(team_totals_data, aes(x=roadWinToLossRatio,y=""))
penalty_plot + 
  geom_boxplot(fill="#cc0000") +
  ggtitle("Boxplot of Road Wins to Losses") +
  labs(x="Ratio of Road Wins, Divided by Road Losses", y="")
```

```{r Creating Histogram of Road Wins to Losses Ratio}
roadWinsLossesHist <- ggplot(team_totals_data, aes(x=roadWinToLossRatio))
roadWinsLossesHist +
...
homeWinsLossesHist <- ggplot(team_totals_data, aes(x=homeWinToLossRatio))
homeWinsLossesHist +
  geom_histogram(aes(y=..density..), binwidth=0.25) + 
  ggtitle("Histogram of Home Wins : Losses Ratio")
```

Is a team that wins when home more likely to win when away? 
```{r}
